...
set_dataset_path(os.environ.get('PB_DIFF_DATASET_DIR', os.path.expanduser('~/.pb_diff/datasets')))
...
class OfflineEnv(gym.Env):
    ...
    def get_dataset(self, h5path=None, no_check=False):
        if h5path is None:
            ...
            if not os.path.exists(self.dataset_url):
                raise ValueError(f"Path Not Exists: {self.dataset_url}")
        ...
        if no_check: # getattr(self, 'is_eval', False):
            return data_dict
        ...
        assert data_dict['actions'].shape[1:] == self.action_space.shape \
                    or data_dict['actions'].shape[1:] == (1,), \
