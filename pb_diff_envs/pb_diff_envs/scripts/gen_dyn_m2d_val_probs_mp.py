...
from pb_diff_envs.utils.dyn.gen_dm2d_val_probs_utils import gen_dm2d_val_probs_mp
...
def append_data_m2d(data, s, a, tgt, done, wall_locations, maze_idx, reach_max_episode_steps, is_sol_kp, 
                    pos_reset, pl_time, n_colchk):
    """
...
def main():
    ...
    parser.add_argument('--is_eval', type=str, default="False", help='', choices=['True', 'False'])

    args = parser.parse_args()
    vis_dir = f'./visualization/{args.env_name}/'
    os.makedirs(name=vis_dir, exist_ok=True)
    args.is_eval = (args.is_eval == 'True')
    assert args.is_eval
    env_dvg_list = gym.make(args.env_name, load_mazeEnv=(args.load_mazeEnv.strip() != "False"), 
                            is_eval=args.is_eval, gen_data=True,)
    
    assert env_dvg_list.eval_num_probs == args.num_samples_permaze, 'equal to env'
    assert env_dvg_list.eval_num_groups == args.num_mazes, 'equal to env'
    ...
    assert args.num_parts <= num_groups
    ...
    if 'DynrandSmaze2d' in args.env_name:
        func_mp = gen_dm2d_val_probs_mp
    else:
        assert not args.is_eval
    ...
    save_to_hdf5_sm2d_val(args, data, env_dvg_list, num_groups, vis_dir)
...
def save_to_hdf5_sm2d_val(args, data, env_dvg_list, num_groups, vis_dir, vis_idx=None, is_ee3d=False):
    '''save the whole dataset'''
    prefix = './datasets'
    env_name = args.env_name if not env_dvg_list.is_eval else args.env_name + '-eval'
    fname = '%s/%s.hdf5' % (prefix, env_name) # noisy is added in env id
    ...
    for i_v in vis_idx:
        ...
        v_start = i_v * env_dvg_list.eval_num_probs
        v_end = v_start + 250
    ...
    if args.gen_rest.strip() == "False":
        # check_wallLoc_matched(data, env_dvg_list) # no need
        load_wallLoc = data['infos/wall_locations'][::env_dvg_list.eval_num_probs]
    ...
    npyname = '%s/%s.npy' % (prefix, env_name)
