...
def gen_dm2d_val_probs_mp(pid, args, env_dvg_list, gen_config, lock, result_queue=None):
    ...
    for maze_idx in range(start_idx, end_idx):
        ...
        while len_env_traj < args.num_samples_permaze:
            ...
            pl_time = np.array( sol_dict['pl_time'] ).reshape(1, 1)
            n_colchk = np.array( sol_dict['n_colchk'] ).reshape(1,1)
            pl_time = np.tile( pl_time, (goal_len, 1) )
            n_colchk =  np.tile( n_colchk, (goal_len, 1) )
        ...
        env_pl_time = np.concatenate( env_pl_time )[:args.num_samples_permaze]
        env_n_colchk = np.concatenate( env_n_colchk )[:args.num_samples_permaze]
