...
def to_torch_tp(*args, dtype=None, device=None):
	'''_tp: return tuple'''
	return (to_torch(arg, dtype, device) for arg in args)
...
def to_torch(x, dtype=None, device=None):
	...
	elif type(x) == list and torch.is_tensor(x[0]):
		return torch.stack(x, dim=0,).to(device=device, dtype=dtype)
